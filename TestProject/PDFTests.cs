...
public class PDFTests
{
  const string apiKey = "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";
    ...
    public async Task ShouldGetConfirmationMessage_WithAddingPDF_WhenTheUrlHasValidPDFExtension()
    {
        ...
        var service = new PDFImplementation(context,new EmbeddingProvide(apiKey),new PromptProvider(new EmbeddingProvide(apiKey)));
    ...
    public async Task ShouldGetErrorMessage_WhenDeletingPDF_WithNoValidExistingURL()
    {
        ...
        Assert.That(result, Is.EqualTo("pdf is not found"));
        await context.Database.EnsureDeletedAsync();
    }
